...
<head>
    ...
    @php
        ...
        $directorSubmissionTrendLabelsSafe = $directorSubmissionTrendLabels ?? ['M1', 'M2', 'M3', 'M4', 'M5', 'M6'];
        $directorTrendPeriodLabelSafe = $directorTrendPeriodLabel ?? 'Last 6 months';
    @endphp
    <script>
        window.soPerformanceData = @json($soPerformanceData ?? []);
        window.directorSubmissionTrendData = @json($directorSubmissionTrendDataSafe);
        window.directorSubmissionTrendLabels = @json($directorSubmissionTrendLabelsSafe);
        window.directorTrendPeriodLabel = @json($directorTrendPeriodLabelSafe);
    </script>
                ...
                <div class="xl:col-span-2 space-y-4 sm:space-y-6">
                    ...
                    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="text-sm font-semibold text-gray-500">{{ $directorTrendPeriodLabelSafe }}</span>
                        </div>
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-7">
                        ...
                        <div class="relative pl-5">
                            ...
                            <div class="space-y-8">
                                @forelse(($recentActivities ?? collect())->take(6) as $activity)
                                    @php
                                        $toneStyles = [
                                            'blue' => ['dot' => 'bg-blue-500', 'badge' => 'bg-blue-50 text-blue-700 border-blue-100'],
                                            'amber' => ['dot' => 'bg-amber-500', 'badge' => 'bg-amber-50 text-amber-700 border-amber-100'],
                                            'yellow' => ['dot' => 'bg-yellow-500', 'badge' => 'bg-yellow-50 text-yellow-700 border-yellow-100'],
                                            'green' => ['dot' => 'bg-emerald-500', 'badge' => 'bg-emerald-50 text-emerald-700 border-emerald-100'],
                                            'red' => ['dot' => 'bg-red-500', 'badge' => 'bg-red-50 text-red-700 border-red-100'],
                                            'slate' => ['dot' => 'bg-slate-400', 'badge' => 'bg-slate-50 text-slate-700 border-slate-100'],
                                        ];

                                        $tone = data_get($activity, 'tone', 'slate');
                                        $dotColor = data_get($toneStyles, $tone.'.dot', 'bg-slate-400');
                                        $badgeClass = data_get($toneStyles, $tone.'.badge', 'bg-slate-50 text-slate-700 border-slate-100');
                                        $title = data_get($activity, 'title', 'Activity update');
                                        $timeStr = data_get($activity, 'time', 'just now');
                                        $context = data_get($activity, 'context', 'Campus');
                                        $descStr = trim((string) data_get($activity, 'description', ''));
                                        $isNotification = data_get($activity, 'source') === 'notification';
                                        $actionLabel = data_get($activity, 'action_label', $isNotification ? 'Notification' : 'Activity');
                                    @endphp
                                    <div class="relative flex items-start">
                                        ...
                                        <div class="ml-5 min-w-0">
                                            ...
                                            <p class="text-[13px] text-slate-400 mb-2">{{ $timeStr }} • {{ $context }}</p>
                                            
                                            @if($descStr)
                                                <p class="text-[14px] text-slate-600 leading-relaxed mb-2">{{ $descStr }}</p>
                                            @endif

                                            <span class="inline-flex items-center px-2 py-0.5 rounded-md border text-[10px] font-semibold {{ $badgeClass }}">
                                                {{ $isNotification ? 'Notification' : $actionLabel }}
                                            </span>
                                        </div>
                                    </div>
                                @empty
                        ...
                        <div class="mt-8 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 text-[13px] text-gray-600">
                            Showing your latest submission events and director-relevant notifications.
                        </div>
                    </div>
